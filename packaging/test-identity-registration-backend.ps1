...
function New-TestPackage {
    param([string]$Path, [string]$Name, [string]$Publisher)
...
function New-FakeAdapter {
    ...
    return @{
        ...
        RemoveCurrentUserPackage = {
            param($fullName)
            $State.Calls.Add('remove-current:GRTech.ezyImageViewer')
            $State.Current['GRTech.ezyImageViewer'] = @()
        }.GetNewClosure()
        RemoveAllUsersPackage = {
            param($fullName)
            $State.Calls.Add('remove-all:GRTech.ezyImageViewer')
            $State.All['GRTech.ezyImageViewer'] = @()
        }.GetNewClosure()
        RemoveProvisionedPackage = {
            param($packageName)
            $State.Calls.Add('deprovision:GRTech.ezyImageViewer')
            $State.Provisioned['GRTech.ezyImageViewer'] = @()
        }.GetNewClosure()
    }
}
...
try {
    ...
    New-TestPackage $external 'GRTech.ezyImageViewer' 'CN=Test Publisher'
    ...
    $state = Invoke-EzyIdentityRegister 'CurrentUser' $install $external $statePath $adapter
    Assert-Backend ([IO.File]::Exists($statePath)) 'Registration state was not persisted.'
    Assert-Backend (($fake.Calls -join '|') -ceq 'add-current:GRTech.ezyImageViewer') `
        'Current-user registration order is invalid.'
    Assert-Backend (@($state.Steps | Where-Object { $_.Introduced }).Count -eq 1) `
        'Introduced package ownership was not recorded.'
    $repair = Invoke-EzyIdentityRegister 'CurrentUser' $install $external $statePath $adapter
    Assert-Backend (@($repair.Steps | Where-Object { $_.Introduced }).Count -eq 1) `
        'Repair lost the original package ownership state.'
    Invoke-EzyIdentityUnregister 'CurrentUser' $install $statePath $adapter
    Assert-Backend ($fake.Calls[-1] -ceq 'remove-current:GRTech.ezyImageViewer') `
        'Current-user removal did not remove the main identity.'
    Assert-Backend (-not [IO.File]::Exists($statePath)) 'Identity state survived uninstall.'

# 이 설치 프로그램이 들여오지 않은 패키지는 제거 뒤에도 살아 있어야 함.
    ...
    Assert-BackendThrows 'unowned main identity' {
        Invoke-EzyIdentityRegister 'CurrentUser' $install $external $statePath `
    ...
    Assert-Backend (($failed.Calls -join '|') -ceq 'add-current:GRTech.ezyImageViewer') `
        'Failed registration recorded an unexpected call sequence.'
    Assert-Backend (-not [IO.File]::Exists($statePath)) `
        'Failed registration persisted ownership state.'

    $all = New-FakeState
    $allAdapter = New-FakeAdapter $all ''
    [void](Invoke-EzyIdentityRegister 'AllUsers' $install $external $statePath $allAdapter)
    Assert-Backend ($all.Calls[0] -ceq 'add-all:GRTech.ezyImageViewer') `
        'All-users registration order is invalid.'
    Assert-Backend ($all.StageRequests[0].Path -ceq $external -and
        $all.StageRequests[0].ExternalLocation -ceq $install) `
        'Main identity all-users stage arguments are invalid.'
    Invoke-EzyIdentityUnregister 'AllUsers' $install $statePath $allAdapter
    Assert-Backend (($all.Calls[1..2] -join '|') -ceq
        'deprovision:GRTech.ezyImageViewer|remove-all:GRTech.ezyImageViewer') `
    ...
    [void](Invoke-EzyIdentityRegister 'CurrentUser' $install $external `
