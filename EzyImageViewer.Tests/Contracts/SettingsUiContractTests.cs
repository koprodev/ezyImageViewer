...
{
    [Fact]
    public void StoreDistribution_RemovesInAppUpdaterAndRegistryAssociationWriters()
    {
        ...
        var dataPaths = File.ReadAllText(RepoFile(
            "EzyImageViewer.Infrastructure", "AppDataPaths.cs"));

        Assert.DoesNotContain("GitHubReleaseUpdateChecker", services, StringComparison.Ordinal);
        Assert.DoesNotContain("CheckForUpdates", settingsUi, StringComparison.Ordinal);
        Assert.DoesNotContain("ShowUpdateAvailableAsync", viewer, StringComparison.Ordinal);
        Assert.DoesNotContain("UpdateCheckStateFile", dataPaths, StringComparison.Ordinal);
        Assert.DoesNotContain("FileAssociationRegistrar", settingsUi, StringComparison.Ordinal);
        Assert.DoesNotContain("UserChoiceDefaultWriter", settingsUi, StringComparison.Ordinal);
        Assert.Contains("BuildPackagedAssociationInfo", settingsUi, StringComparison.Ordinal);
        Assert.Contains(
    ...
    [Fact]
    public void StoreDistribution_HasNoBinaryDownloadInstallOrRegistryWriteImplementation()
    {
