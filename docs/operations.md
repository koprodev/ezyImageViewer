...
로그와 복구 파일을 서버로 자동 업로드하는 telemetry·원격 수집 파이프라인과 자동 버전
확인 기능은 구현되어 있지 않다. 사용자가 `업데이트 확인`을 누르면 앱은 고정 GitHub
Releases 주소를 OS 기본 브라우저에 전달할 뿐이며, 실패한 shell launch의 분류만 로컬 로그에
기록할 수 있다.
